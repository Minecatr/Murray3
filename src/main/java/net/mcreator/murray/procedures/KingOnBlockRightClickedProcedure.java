...
public class KingOnBlockRightClickedProcedure {
		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == Items.QUARTZ) {
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.player.burp")),
			...
			world.setBlock(new BlockPos(x, y + 2, z), MurrayModBlocks.PARTICLE.get().defaultBlockState(), 3);
		}
	}
}
