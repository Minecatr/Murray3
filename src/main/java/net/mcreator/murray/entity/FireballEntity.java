...
import net.mcreator.murray.procedures.FireballWhileProjectileFlyingTickProcedure;
import net.mcreator.murray.procedures.FireballProjectileHitsLivingEntityProcedure;
...
public class FireballEntity extends AbstractArrow implements ItemSupplier {
	public FireballEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(MurrayModEntities.FIREBALL.get(), world);
	}
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		FireballProjectileHitsLivingEntityProcedure.execute(entityHitResult.getEntity());
	}

	@Override
	public void tick() {
		super.tick();
		FireballWhileProjectileFlyingTickProcedure.execute(this.level, this.getX(), this.getY(), this.getZ());
		if (this.inGround)
			this.discard();
	}

	public static FireballEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
		FireballEntity entityarrow = new FireballEntity(MurrayModEntities.FIREBALL.get(), entity, world);
	...
	public static FireballEntity shoot(LivingEntity entity, LivingEntity target) {
		FireballEntity entityarrow = new FireballEntity(MurrayModEntities.FIREBALL.get(), entity, entity.level);
